...
if ifDenoi == 1
    ...
    % % Denoise
    % [u, dgeFun] = mlsvd(imgCsf, [medInd(1), medInd(2), medInd(3)]);
    [u, dgeFun] = mlsvd(imgCsf, round([xn/2, yn/2, tn/3*(yn/tn)]));
    imgCsf = lmlragen(u, dgeFun);
end
...
%% 
baseImgAir = mean(imgCsf(:,:,(baseO2Time+baseTransTime)*4+1:(baseO2Time+baseTransTime)*4+baseO2Num),3);
baseImgO2 = mean(imgCsf(:,:,1:baseO2Num),3);
for nd = 1:tn
    imgDge(:,:,nd)=(baseImgO2-imgCsf(:,:,nd))./baseImgO2*100;
